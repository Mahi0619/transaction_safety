...
object BiometricPromptUtils {
    ...
    fun createFaceRecognitionPrompt(activity: AppCompatActivity): BiometricPrompt.PromptInfo =
        BiometricPrompt.PromptInfo.Builder().apply {
            setTitle(activity.getString(R.string.prompt_info_title))
            setAllowedAuthenticators(BiometricManager.Authenticators.BIOMETRIC_STRONG)
            setConfirmationRequired(false)
    ...
    fun createPinPrompt(activity: AppCompatActivity): BiometricPrompt.PromptInfo =
        BiometricPrompt.PromptInfo.Builder().apply {
            setTitle(activity.getString(R.string.prompt_info_title))
            setAllowedAuthenticators(BiometricManager.Authenticators.DEVICE_CREDENTIAL)
            setConfirmationRequired(false)
    ...
    fun createFingerprintPrompt(activity: AppCompatActivity): BiometricPrompt.PromptInfo =
        BiometricPrompt.PromptInfo.Builder().apply {
            setTitle(activity.getString(R.string.prompt_info_title))
            setAllowedAuthenticators(BiometricManager.Authenticators.BIOMETRIC_WEAK)
            setConfirmationRequired(false)
    ...
    fun authenticate(
        ...
        val promptInfo = if (canAuthenticateWithBiometrics(activity)) {
            createDefaultPrompt(activity)
        } else {
            createDeviceCredentialPrompt(activity)
        }
